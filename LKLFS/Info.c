  ...
  )
{
  ...
  // write name
  Status = AsciiStrToUnicodeStrS(FileName, FileInfo->FileName, NameSize/sizeof(CHAR16));
  if (EFI_ERROR(Status)) {
    return Status;
  }
  ...
  )
{
  ...
  if (*BufferSize >= ResultSize) {
    ...
    Status = AsciiStrToUnicodeStrS(Volume->FsType, Buffer + Size, NameSize/sizeof(CHAR16));
  }
