...
{
  UINTN DestMax = StrLen (UnicodeStr) + 1;

  CHAR8* AsciiStr = AllocatePool(DestMax * sizeof (CHAR8));
  if (AsciiStr == NULL) {
    return NULL;
  }

  UnicodeStrToAsciiStrS(UnicodeStr, AsciiStr, DestMax);
...
{
  UINTN DestMax = AsciiStrLen (AsciiStr) + 1;

  CHAR16* UnicodeStr = AllocatePool(DestMax * sizeof (CHAR16));
  if (UnicodeStr == NULL) {
    return NULL;
  }

  AsciiStrToUnicodeStrS(AsciiStr, UnicodeStr, DestMax);
